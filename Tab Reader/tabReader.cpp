...
void stringToArray(char are[],char arB[], char arG[], char arD [], 
	char arA[], char arE[], string e, string B, string G, string D, 
	string A, string E);
...
void mergeStrings(string topString, string midString, 
	string lowString, string & final, bool top);	
...
int main() //program assumes tablature is written correctly
{
	...
	string rawe = "|", rawB = "|", rawG= "|", rawD= "|", rawA= "|"
	string rawE = "|"; // Different declaration for printing purposes
	string tuning [6];

	getTab(rawe, rawB, rawG, rawD, rawA, rawE, tuning, fileName, fin);
	
	char are[MAX_SIZE], arB[MAX_SIZE], arG[MAX_SIZE], arD[MAX_SIZE],
	 arA[MAX_SIZE], arE[MAX_SIZE];
	
	string e, B, G, D, A, E;
	
	string_init(e,B,G,D,A,E,rawe,rawB,rawG,rawD,rawA,rawE,are, arB, 
		arG,arD,arA,arE);
	...
	cout << endl<<  "Tab conversion successful";	
		...
		char arD [], char arA[], char arE[])
{
	...
	stringToArray(are, arB, arG, arD, arA, arE, rawe, rawB, rawG, rawD, rawA, 
	rawE);
...
// --The following code pertains only to the current to string prototype-------
